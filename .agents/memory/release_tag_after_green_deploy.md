...
- The workflow pins one immutable master SHA, runs Full Suite once, publishes and smoke-tests the self-hosted community image/assets, deploys hosted SaaS from that same SHA (default: public `Deploy hosted SaaS` lane; optional private operations dispatch), waits for production deploy and smoke checks, and only then makes the GitHub release public.
