...
- The workflow pins one immutable master SHA, runs Full Suite once, publishes and smoke-tests the self-hosted community image/assets, hands the same exact SHA to the private operations repository, waits for its production deploy and smoke checks, and only then makes the GitHub release public.
- A GitHub release or tag alone is never proof that SaaS deployed. Only a successful canonical `Release` whose correlated private deployment completed for the same SHA is the public deployed-SHA marker.
