...
**Rule:** Keep Vercel Git auto-deploy disabled for SaaS frontend projects. Production frontend deploys for `app.genfeed.ai`, `genfeed.ai`, `docs.genfeed.ai`, and `marketplace.genfeed.ai` run through the public `Deploy hosted SaaS` / `Release` workflows after the production API release succeeds. The deploy implementation stays in the private operations repository and is invoked as a reusable workflow so compute bills the public caller. Each repository is deployed from an exact SHA verified against its trunk. There is no standalone `admin.genfeed.ai` Vercel project in the open-source monorepo; instance-admin UI should live under `apps/app` when it is exposed.
...
- Deploy production frontends only from the public `Release` / `Deploy hosted SaaS` lane (or the explicit `operations` fallback) after the production API deployment succeeds.
