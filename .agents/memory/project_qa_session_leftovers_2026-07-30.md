...
8. **Docs SEO test hardcodes the docs page count** — GH #2444. `seo-metadata` test pins an exact page count and breaks on every docs addition; derive the expectation from the filesystem instead. Fold into the next train, not its own branch
