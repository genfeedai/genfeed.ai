...
- **Merging goes through the GitHub merge queue** (#3143, ruleset "Passing CI on master": SQUASH, HEADGREEN, build 5 / merge 1–5, timeout 60 min): use "Merge when ready" / `gh pr merge --squash --auto`; never bypass the queue for a routine merge. Every required context re-runs on the queue's `gh-readonly-queue/master/...` commit via `merge_group` triggers (`ci.yml`, `pr-title.yml`); `merge-queue-external-checks.yml` republishes the PR-enforced `license/cla` and `Socket Security: Project Report` contexts there. Adding a required check means adding a `merge_group` path for it, or the queue times out.
