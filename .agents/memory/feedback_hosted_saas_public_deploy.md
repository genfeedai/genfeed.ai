...
**Why:** Private console Actions minutes are paid. Public runners are free. GitHub Team cannot `uses:` a private reusable workflow from this public repo — dispatch fails with `workflow was not found`. Console keeps OpenTofu, fleet, and LoRA; the public engine checks console out at runtime.

**How to apply:**
- Default `saas_lane=monorepo`. Jobs live in `.github/workflows/_deploy-hosted-saas-core.yml`.
- Do not add `uses: genfeedai/console.genfeed.ai/.github/workflows/...`.
- Do not copy OpenTofu or fleet/LoRA into this public tree.
