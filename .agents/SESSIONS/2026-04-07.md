...
- **Mistake:** Unused `ButtonVariant` import left in cloud-content.tsx after removing "Self-Host Instead" button → **Fix:** Removed unused import before commit
- **Mistake:** Unused `TRAINING_PACKAGES` import in services-content.tsx → **Fix:** Removed unused import before commit
- **Mistake:** Pre-existing biome errors (`noNonNullAssertion`, `noStaticElementInteractions`) blocked commit → **Fix:** Added biome-ignore comments with explanations
- **Mistake:** Committed on wrong branch (`chore/fix-package-publish`) → **Fix:** Cherry-picked commit onto `develop`

### Commits

1. `7f743404` — feat(website): restructure pricing to subscription model + cal.com playbook (23 files)

### Next steps

- [ ] Push to develop and verify CI passes
- [ ] Visual QA: `bun dev:app @genfeedai/website` — check homepage, /pricing, /services, /cloud, /features
- [ ] Consider adding annual pricing toggle (already in app PricingSubscriptions component)

## Session 5: One API Epic — Self-Hosted + Cloud Unification (Epic #95)
...
```mermaid
flowchart TD
    A[User visits /] --> B{IS_SELF_HOSTED?}
    B -->|Yes| C[CombinedAuthGuard bypass]
    B -->|No| D[Clerk JWT auth]
    C --> E[Request-context middleware loads seeded user from DB]
    D --> F[Request-context middleware loads Clerk user]
    E --> G[req.context with valid ObjectIds]
    F --> G
    G --> H[NestJS controllers serve request]
    
    I[Self-hosted install] --> J{Method?}
    J -->|npx| K[npx @genfeedai/create]
    J -->|Docker| L[docker run ghcr.io/genfeedai/genfeed.ai]
    J -->|Compose| M[docker compose up]
    K --> N[Persistent MongoDB + Redis]
    L --> N
    M --> N
    N --> O[Seed default org/brand/user]
    O --> P[App running on localhost]
    
    Q[Release created on GitHub] --> R[deploy-production.yml]
    R --> S[Deploy backend to EC2]
    S --> T[Deploy 4 Vercel apps in parallel]
```
...
| Layer | Components |
|-------|------------|
| Frontend | `apps/app` (proxy.ts, next.config.ts, useOptionalAuth, AppProviders, protected-layout-client, edition badge), `apps/docs` (migrated from separate repo), `packages/create` (migrated + updated) |
| Backend | `apps/server/api` (combined-auth.guard, request-context.middleware, clerk.provider, stripe.service, clerk.strategy, clerk.guard, webhook controllers, seed service, core controllers), `apps/server/files`, `apps/server/notifications`, `apps/server/workers` |
| Config | `packages/config` (edition.ts, helpers.ts, all schema files conditional), `packages/storage` (new Local+S3 provider) |
| WebSocket | `packages/libs/websockets` (room-name.util.ts, gateway, service), 28 files with getUserRoomName() |
| Docker | `docker/Dockerfile.selfhosted`, `docker/selfhosted-entrypoint.sh`, `docker/docker-compose.selfhosted.yml` |
| CI/CD | `.github/workflows/deploy-production.yml` (release trigger + Vercel), `.github/workflows/docker-publish.yml`, `.github/workflows/ci.yml` (format check), `.github/workflows/codebase-health.yml` (fallow) |
| Data | `packages/types/src/streak.ts` (missing migration fix), `MONGODB_URL` → `MONGODB_URI` (15 files) |
| Ports | 107 files consolidated to 300x/301x/302x/303x scheme |

### What was done

- [x] Evaluated fallow codebase health tool, ran initial analysis (score 72/100), created CI workflow (#83)
- [x] Confirmed cloud+core→genfeed.ai migration is complete (114 pages, 3871 tests)
- [x] Designed One API architecture with Cal.com as reference model
- [x] Created epic #95 with 20 sub-issues across 8 phases
- [x] Ran 5 rounds of Codex adversarial reviews, fixing all findings
- [x] **Phase 0:** Made all config schemas conditional (Stripe, ElevenLabs, AWS, AI, social, Sentry) — `conditionalRequired()` helper
- [x] **Phase 0:** Made User.clerkId optional (sparse unique index), added isDefault to Org/User/Brand schemas
- [x] **Phase 0.5:** Noop Clerk/Stripe providers, webhook controller guards
- [x] **Phase 1:** Edition detection package (IS_SELF_HOSTED, IS_CLOUD, IS_EE), @CloudOnly/@SelfHostedOnly decorators
- [x] **Phase 1:** CombinedAuthGuard self-hosted bypass
- [x] **Phase 1:** Request-context middleware self-hosted path (loads seeded user from DB)
- [x] **Phase 1:** Socket.IO room naming utility (getUserRoomName) — 28 files updated
- [x] **Phase 2:** Self-hosted seed service (OnApplicationBootstrap, idempotent)
- [x] **Phase 3:** MONGODB_URL → MONGODB_URI rename (53 occurrences, 15 files)
- [x] **Phase 4:** Deleted all 16 Next.js API routes, created 4 NestJS core controllers
- [x] **Phase 4:** Next.js proxy rewrite (/v1/* → NestJS), WebSocket direct connection
- [x] **Phase 4:** Webhook vs polling edition switch for Replicate
- [x] **Phase 5:** Edition-aware frontend layouts (conditional ClerkProvider, useOptionalAuth hook)
- [x] **Phase 6:** @genfeedai/storage package (LocalStorageProvider + S3StorageProvider)
- [x] **Phase 7:** Self-hosted Docker image (Dockerfile.selfhosted, tini, mongod+redis embedded)
- [x] **Phase 7:** CI workflow for GHCR image publishing
- [x] **Phase 7:** Self-hosted seed service recreated (lost in merges)
- [x] Fixed streak bug (missing streak.ts types from cloud migration)
- [x] Fixed onboarding redirect loop (/onboarding prefix matching)
- [x] Fixed useOptionalAuth (static import instead of require)
- [x] Port consolidation — 107 files to logical 300x/301x/302x/303x scheme
- [x] Migrated docs.genfeed.ai into apps/docs/
- [x] Migrated @genfeedai/create into packages/create/ (persistent MongoDB, Redis, new ports)
- [x] Deploy production changed to release-only trigger (not push to master)
- [x] Vercel auto-deploy disabled on all 6 projects, deploys via GitHub Action
- [x] CI format check added (biome format:check)
- [x] Vercel repo variables + secret configured
- [x] Created issue #131 for secret migration (assigned to user)
- [x] Created on-develop label for tracking pre-release issues
- [x] Created BullMQ processor migration issue (#84)

### Files changed

~200+ files total across all phases. Key new files:
- `packages/config/src/helpers.ts` — conditionalRequired(), IS_SELF_HOSTED
- `packages/config/src/edition.ts` — IS_CLOUD, IS_EE
- `packages/storage/` — new package (6 files)
- `packages/create/` — migrated + rewritten
- `apps/docs/` — migrated from separate repo
- `apps/server/api/src/seeds/self-hosted-seed.service.ts` + module
- `apps/server/api/src/endpoints/core/` — 4 new controllers + module
- `apps/server/api/src/helpers/decorators/edition/edition.decorator.ts`
- `apps/app/src/hooks/useOptionalAuth.ts`
- `apps/app/src/components/edition-badge/EditionBadge.tsx`
- `packages/libs/websockets/room-name.util.ts`
- `packages/types/src/streak.ts`
...
- **Decision:** One NestJS API for both self-hosted and cloud, not separate APIs
  - **Context:** User wanted Cal.com model — same repo, same code, Clerk connection upgrades features
  - **Rationale:** Desktop already uses NestJS. Eliminates 16 redundant Next.js routes. Single auth flow.

- **Decision:** Noop providers instead of conditional module imports
  - **Context:** NestJS DI crashes at construction time when Clerk/Stripe keys missing
  - **Rationale:** Noop providers preserve injection tokens — guards/middleware can inject without conditional checks. Less invasive than restructuring module graph.

- **Decision:** Next.js URL rewrites for self-hosted clean URLs
  - **Context:** Self-hosted users don't have org/brand slugs in URLs
  - **Rationale:** Zero page duplication — same [orgSlug]/[brandSlug] pages serve both modes. Rewrites map /workspace/overview → /default/default/workspace/overview.

- **Decision:** Ports 300x for frontend, 301x for backend
  - **Context:** Ports were scattered (3001, 3005, 3007, 3102, 4001)
  - **Rationale:** Logical grouping. Avoids port 3000 collision with files service fallback.

- **Decision:** Deploy on release only, not push to master
  - **Context:** Merging develop→master would auto-deploy to production
  - **Rationale:** Matches Cal.com pattern. Backend + frontend deploy together via single release.

- **Decision:** Vercel auto-deploy disabled, deploys via GitHub Action
  - **Context:** Need coordinated deploy of backend (EC2) + frontend (Vercel)
  - **Rationale:** One release = one coordinated deploy. No drift between backend and frontend versions.

### Mistakes and fixes

- **Mistake:** useOptionalAuth used require('@clerk/nextjs') dynamically → **Fix:** Changed to static import (require creates separate module instance in SSR)
- **Mistake:** middleware.ts redirected to /workspace/overview (doesn't exist) → **Fix:** Deleted middleware.ts, added self-hosted URL rewrites
- **Mistake:** Self-hosted seed service lost in PR merges → **Fix:** Recreated with proper DB connections
- **Mistake:** streak.ts types not migrated from cloud → **Fix:** Created packages/types/src/streak.ts
- **Mistake:** /onboarding redirect loop (trailing slash mismatch) → **Fix:** Changed skipRedirectPrefixes from '/onboarding/' to '/onboarding'
- **Mistake:** Port 3000 assigned to web app but hardcoded as files service fallback in 7 files → **Fix:** Moved to 4000-range (later consolidated to 300x/301x)

### Next steps

- [ ] Copy 12 GitHub secrets from cloud repo (#131) — blocking first release
- [ ] Merge PR #133 (docs refresh)
- [ ] Merge develop → master
- [ ] Create first release (triggers deploy)
- [ ] Move 32 BullMQ processors from API to Workers (#84)
- [ ] Deduplicate triplicated UI packages (#122)
- [ ] Fix @genfeedai/types build (#121)
- [ ] Update Vercel project git sources from cloud → genfeed.ai repo
