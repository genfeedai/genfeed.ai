...
- `packages/ui/web-tokens.css` — generated semantic theme variables and invariant tokens.
- `packages/styles/globals.css` — Tailwind v4 theme mapping and global UI behavior.
- `packages/styles/shadcn-theme.css` — shadcn-compatible variables, overlays, platform colors, effects.
...
6. Run `bun run design:check` after editing `DESIGN.md`, tokens, theme mappings, or the design evaluation seed.
...
- `scripts/design-eval/` freezes matched agent-evaluation scenarios. Keep the prompt, inputs, model, viewport, and first-attempt policy identical across a pair; guidance is the only variable.
