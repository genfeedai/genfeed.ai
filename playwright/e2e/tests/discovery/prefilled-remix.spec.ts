...
test.describe('Discovery prefilled remix handoff', () => {
  test('takes an eligible TikTok trend through Review approval to a Publishing draft', async ({
    ...
    await expect(reviewLink).toHaveAttribute(
      'href',
      /\/publishing\/review\?batch=review-batch-1$/,
    );
  ...
  test('links an approved organic TikTok run to its Publishing drafts downstream', async ({
    authenticatedPage,
  }) => {
    const baseRun = buildRun({
      id: 'run-publish-1',
    ...
    const panel = authenticatedPage.getByRole('region', { name: 'Remix run' });
    await expect(panel).toBeVisible();
    const publishingDrafts = panel.getByRole('link', {
      name: 'Open Publishing drafts',
    });
