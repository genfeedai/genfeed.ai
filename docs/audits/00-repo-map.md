...
- **Both packages are real:** `billing` (`@genfeedai/ee-billing`, 33 files) and `harness` (`@genfeedai/ee-harness`, 1 file — a single `ContentHarnessPack` plugging into OSS `packages/harness`'s registry).
- **Placeholders resolved:** `teams` deleted in PR #1083 (keep/kill grep found no enterprise-teams code); `analytics` and `multi-tenancy` deleted in #1093 — business analytics stayed OSS (Stripe-preferred revenue in `apps/server/api/src/endpoints/analytics/business-analytics.service.ts`, the duplicate license-gated collection module was an unreachable shadowed route and was deleted), and multi-tenancy enforcement (`CombinedAuthGuard` at `apps/server/api/src/helpers/guards/combined-auth/`, 301 `organizationId` references in the Prisma schema) stays in the OSS API by design — no separable EE code unit exists.
...
2. **Backend lint coverage is near-zero.** Biome linting is disabled for all of `apps/server/**`; only 7 warn-rules apply to *staged* files at commit time. ~1,400 spec files notwithstanding, nothing enforces `noExplicitAny`, unused-vars, etc. on the largest code surface (`biome.json:104-114`, `biome-staged.json`). *Remediated in this PR:* linter reactivated for `apps/server/**` (only `useImportType` off — NestJS `emitDecoratorMetadata` requires value imports, same rationale as the existing `ee/**` override), all 57 error-level violations repo-wide fixed, `biome-staged.json` weakening override removed. Residual backlog: 1,777 warnings repo-wide (topped by `noExplicitAny` 933, `noNonNullAssertion` 361) — ratchet these in audit session 04.
...
5. **Docs/memory describe a target state the code doesn't match.** *(Remediated: `teams` placeholder deleted in PR #1083; `analytics` + `multi-tenancy` placeholders deleted and steering docs corrected in #1093 — multi-tenancy and business analytics are OSS by design.)* Residual: a root `e2e/` dir is referenced but doesn't exist. In an AI-writes-all-code shop, stale steering docs directly cause wrong future code.
