...
| Workflow contracts shim | jscpd reports `packages/workflow-engine/src/workflows-contracts-shim.ts` vs `packages/workflows/src/contracts/execution.ts` | Shim may exist to avoid dependency cycles or preserve package API. Confirm dependency graph before deletion. |
