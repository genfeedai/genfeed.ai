...
3. Draft a structured report matching `.github/ISSUE_TEMPLATE/bug.yml` — title,
   ...
   never invented. Pasted errors/stack traces are quoted verbatim.
4. Redact before filing. Scan the drafted report — pasted errors and stack traces
   especially — for secrets, credentials, tokens, personal or customer data, and
   undisclosed security-vulnerability detail. Replace what you find with a
   placeholder. If sensitive content cannot be removed without gutting the report,
   stop and say so instead of filing — this repo is public.
