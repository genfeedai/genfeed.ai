...
module "service" {
  ...
  task_role_arn      = contains(local.public_backend_service_names, each.key) ? aws_iam_role.public_task.arn : aws_iam_role.task.arn

  subnets            = local.private_subnet_ids
  security_group_ids = [aws_security_group.ecs.id]
  namespace_id       = aws_service_discovery_private_dns_namespace.internal.id

  secrets = contains(local.public_backend_service_names, each.key) ? local.public_backend_task_secrets[each.key] : local.service_task_secrets
...
# ── One-off launch-articles seed ─────────────────────────────────────────────
# The public application image contains a compiled, idempotent article seed.
# Keep its default command dry-run and tenant-pinned so an operator cannot write
# to the oldest arbitrary organization by launching the task definition alone.
resource "aws_cloudwatch_log_group" "articles_seed" {
  name              = "/ecs/${local.name_prefix}/articles-seed"
  retention_in_days = 7
}

resource "aws_ecs_task_definition" "articles_seed" {
  family                   = "${local.name_prefix}-articles-seed"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = local.services.api.cpu
  memory                   = local.services.api.mem
  ...
  container_definitions = jsonencode([{
    ...
    environment = concat(local.internal_env, [
      { name = "PORT", value = tostring(local.services.api.port) },
      { name = "SERVICE_NAME", value = "articles-seed" },
    ])
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        "awslogs-group"         = aws_cloudwatch_log_group.articles_seed.name
        "awslogs-region"        = var.region
        "awslogs-stream-prefix" = "articles-seed"
      }
    }
  }])
}
