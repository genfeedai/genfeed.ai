...
function wrapper({ children }: { readonly children: ReactNode }) {
  return <DiscoverRemixProvider>{children}</DiscoverRemixProvider>;
}

describe('DiscoverRemixProvider', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('allows preparation after a brand becomes available', async () => {
    mocks.brandId.value = '';
    const { rerender, result } = renderHook(() => useDiscoverRemix(), {
      wrapper,
    });
