...
/**
 * Posts from the brand's own connected accounts (imported at connect time,
 * not generated by Genfeed) surface under `owned`; followed creators stay
 * under `following`.
 */
export function toDeskItemFromSourcePost(
  post: ISourcePost,
  options: { isOwnAccount?: boolean } = {},
): DiscoveryDeskItem {
  const metrics: DiscoveryDeskItemMetrics = {
  ...
  return {
    ...
    remixSelector: opensPrefilledRemix
      ...
      : null,
    source: options.isOwnAccount ? 'owned' : 'following',
