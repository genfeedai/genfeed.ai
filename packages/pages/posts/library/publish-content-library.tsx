...
import PublishContentLibraryToolbar from '@pages/posts/library/PublishContentLibraryToolbar';
import {
  createPublishContentLibraryItems,
  filterPublishContentLibraryItems,
...
interface PublishContentCollections {
  articles: Article[];
  newsletters: Newsletter[];
  posts: IPost[];
}

const EMPTY_COLLECTIONS: PublishContentCollections = {
  articles: [],
  newsletters: [],
  posts: [],
};
...
export default function PublishContentLibrary() {
  ...
  const {
    ...
    queryKey: ['publish-content-library', organizationId, brandId],
  });
  ...
  useEffect(() => {
    ...
    logger.error('Failed to load publish content library', error);
    notificationsService.error('Failed to load content library');
  }, [error, notificationsService]);

  const items = useMemo(
    () => createPublishContentLibraryItems(collections),
    [collections],
  );
  const channelOptions = useMemo(
    () =>
      [...new Set(items.map((item) => item.channel))]
        .map((value) => ({
          label: formatPublishContentChannel(value),
          value,
        }))
  ...
  const statusOptions = useMemo(
    () =>
      [...new Set(items.map((item) => item.status))]
        .map((value) => ({
          label: formatPublishContentStatus(value),
          value,
        }))
        .sort((left, right) => left.label.localeCompare(right.label)),
    [items],
  ...
  const status =
    ...
    statusOptions.some((option) => option.value === requestedStatus)
      ? requestedStatus
      : 'all';
  const type = parsePublishContentType(parsedSearchParams.get('type'));
  ...
  const filteredItems = useMemo(
    () =>
      filterPublishContentLibraryItems(items, {
        channel,
        search,
        status,
        type,
  ...
  const columns = useMemo<TableColumn<PublishContentLibraryItem>[]>(
    () => [
      ...
      },
      {
        header: 'Type',
        key: 'type',
        render: (item) => <Badge>{formatPublishContentType(item.type)}</Badge>,
      },
      {
        header: 'Channel',
        key: 'channel',
        render: (item) => formatPublishContentChannel(item.channel),
      },
      {
        header: 'Status',
        key: 'status',
  ...
  return (
    <div>
      ...
      <AppTable<PublishContentLibraryItem>
        actions={[]}
        columns={columns}
