...
import { dispatchWorkspaceInput, selectHistoryEntry } from '../../src/tui/workspace-dispatch';
...
describe('TUI workspace dispatch', () => {
  it('dispatches a parsed slash command to the operation handler', async () => {
    ...
    await dispatchWorkspaceInput('/workflow run weekly-content', {
      appendError: vi.fn(),
      runMessage: vi.fn(),
      runOperation,
    });
  ...
  it('appends operation failures as terminal errors', async () => {
    ...
    await dispatchWorkspaceInput('/balance', {
      appendError,
      runMessage: vi.fn(),
