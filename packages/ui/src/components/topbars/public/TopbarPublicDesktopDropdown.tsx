...
export default function TopbarPublicDesktopDropdown({
    ...
    return (
      <li key={item.href}>
        <Link
          href={item.href}
          className={cn(
            'group flex min-h-20 items-start gap-3 rounded-lg border border-transparent px-4 py-3.5 transition-[background-color,border-color,color]',
            isActive
              ? 'border-edge/10 bg-foreground/8 text-foreground'
              : 'text-foreground/75 hover:border-edge/10 hover:bg-foreground/[0.035] hover:text-foreground',
          )}
          onClick={onItemClick}
        >
          {Icon && (
            <span className="flex size-8 shrink-0 items-center justify-center rounded-md border border-edge/10 bg-background/70">
              <Icon className="size-4 text-foreground/65 transition-colors group-hover:text-foreground" />
            </span>
          )}
  ...
  return createPortal(
    ...
    >
      {hasGroups ? (
        <div className="w-screen border-y border-edge/10 bg-background/98 shadow-[0_28px_80px_rgba(0,0,0,0.34)] backdrop-blur-2xl">
          <div className="container mx-auto grid grid-cols-3 gap-7 px-6 py-6">
            {groupItems(currentDropdown.items).map(([groupLabel, items]) => (
              <div className="min-w-0" key={groupLabel}>
                {groupLabel && (
                  <div className="px-4 pb-2 pt-1 text-[10px] font-bold uppercase tracking-[0.16em] text-foreground/35">
                    {groupLabel}
                  </div>
                )}
            ...
            <div className="col-span-3 flex items-center justify-between border-t border-edge/10 px-4 pt-5">
