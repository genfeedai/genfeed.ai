...
const settings = { organizationEnabled: true, brandEnabled: null, brandId: 'brand-1', isEnabled: true, source: 'organization' as const };
function props() { return { brandId: 'brand-1', error: null, isLoading: false, isSaving: false, onRefresh: vi.fn(), onSave: vi.fn().mockResolvedValue(undefined), settings }; }
...
describe('GenerationHarnessSettingsCard', () => {
  it('offers a distinct brand override and inheritance without changing the organization', () => {
    ...
    const group = within(screen.getByRole('group', { name: 'Brand prompt enhancement' }));
    expect(group.getByRole('button', { name: 'Inherit' })).toHaveAttribute('aria-pressed', 'true');
  ...
  it('exposes organization default and reset with accessible names', () => {
    const input = props();
    render(<GenerationHarnessSettingsCard {...input} />);
    fireEvent.click(screen.getByRole('switch', { name: 'Organization prompt enhancement' }));
    expect(input.onSave).toHaveBeenCalledWith('organization', false);
    fireEvent.click(screen.getByRole('button', { name: 'Reset organization default' }));
    expect(input.onSave).toHaveBeenCalledWith('organization', null);
  });
  it('disables writes while saving and makes an error recoverable', () => {
    const input = props();
    const { rerender } = render(<GenerationHarnessSettingsCard {...input} isSaving />);
