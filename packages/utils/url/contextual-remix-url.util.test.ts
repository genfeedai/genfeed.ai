...
describe('contextual-remix-url.util', () => {
  ...
  it('preserves opaque route state and pins the selected record version', () => {
    expect(
      buildContextualRemixHref(
        '/acme/moonrise/studio/generate?thread=thread-1&overlay=library-picker',
        reference,
      ),
    ).toBe(
      '/acme/moonrise/studio/generate?thread=thread-1&sourceArtifact=ingredient%3Aingredient-1&sourceVersion=7',
    );
  });
});
