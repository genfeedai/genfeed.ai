...
export function buildSystemEmailParagraph(text: string): string {
  return `<p style="margin:0 0 16px;color:#A1A1A1;font-size:15px;line-height:24px;">${escapeSystemEmailHtml(text)}</p>`;
}
...
export function buildSystemEmailHtml(input: SystemEmailOptions): string {
  ...
  const brandMark = appUrl
    ? `<a href="${escapeSystemEmailHtml(appUrl)}" style="color:#EDEDED;font-size:14px;font-weight:700;letter-spacing:0;text-decoration:none;">${BRAND_NAME}</a>`
    : `<span style="color:#EDEDED;font-size:14px;font-weight:700;letter-spacing:0;">${BRAND_NAME}</span>`;
  const footerLink = appUrl
    ? ` <a href="${escapeSystemEmailHtml(appUrl)}" style="color:#A1A1A1;text-decoration:underline;">Open Genfeed</a>`
    : '';
  const preheader =
    input.preheader ??
  ...
  const action =
    input.action && actionUrl
      ? `<tr><td style="padding:8px 32px 24px;"><a href="${escapeSystemEmailHtml(actionUrl)}" style="background:#EDEDED;border-radius:8px;color:#0A0A0A;display:inline-block;font-size:14px;font-weight:700;line-height:20px;padding:12px 18px;text-decoration:none;">${escapeSystemEmailHtml(input.action.label)}</a></td></tr>`
      : '';
  const footerNote = input.footerNote
    ? `<p style="margin:0 0 12px;color:#949494;font-size:12px;line-height:18px;">${escapeSystemEmailHtml(input.footerNote)}</p>`
    : '';
...
<html lang="en">
  ...
  <body style="background:#0A0A0A;margin:0;padding:0;">
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#0A0A0A;border-collapse:collapse;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;margin:0;padding:0;width:100%;">
      <tr>
        <td align="center" style="padding:32px 16px;">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;max-width:600px;width:100%;">
            ...
            <tr>
              <td style="background:#161616;border:1px solid #333333;border-radius:10px;padding:0;">
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;width:100%;">
                  <tr>
                    <td style="padding:32px 32px 8px;">
                      <div style="color:#949494;font-size:12px;font-weight:700;letter-spacing:.08em;line-height:16px;text-transform:uppercase;">${escapeSystemEmailHtml(eyebrow)}</div>
                    </td>
                  </tr>
                  <tr>
                    <td style="padding:0 32px 14px;">
                      <h1 style="color:#EDEDED;font-size:24px;font-weight:700;letter-spacing:0;line-height:30px;margin:0;">${escapeSystemEmailHtml(input.title)}</h1>
                    </td>
                  </tr>
                  <tr>
                    <td style="color:#A1A1A1;font-size:15px;line-height:24px;padding:0 32px;">
                      ${input.bodyHtml}
                    </td>
                  </tr>
                  ${action}
            ...
            <tr>
              <td style="padding:18px 4px 0;">
                ${footerNote}
                <p style="margin:0;color:#949494;font-size:12px;line-height:18px;">This message was sent by ${BRAND_NAME}.${footerLink}</p>
              </td>
            </tr>
          </table>
        </td>
