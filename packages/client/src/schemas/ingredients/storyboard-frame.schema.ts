 ...
 */
export function initializeStoryboard(format: IngredientFormat): Storyboard {
 ...
 */
export function isGenerating(storyboard: Storyboard): boolean {
  return storyboard.frames.some(
    (frame: StoryboardFrame) => frame.status === 'generating',
  );
}
