...
Skill invocation routes through immutable workflow executions, so the representative
verification path is split across the request boundary and the runner:

- [`apps/server/api/src/services/content-gateway/content-gateway.controller.spec.ts`](https://github.com/genfeedai/genfeed.ai/blob/master/apps/server/api/src/services/content-gateway/content-gateway.controller.spec.ts)
- [`apps/server/server/src/collections/workflows/system-workflow-runner.service.spec.ts`](https://github.com/genfeedai/genfeed.ai/blob/master/apps/server/server/src/collections/workflows/system-workflow-runner.service.spec.ts)

Both use mocked provider services so they run in CI without live credentials, while keeping
the production wiring:

- the gateway controller routes an inbound signal and executes a manual skill request
- organization scope is resolved from the authenticated user rather than the request body
- the manual request is handed to the workflow runner as an immutable execution
- the runner resolves the workflow, advances its nodes, and records execution state
- generated output is serialized before it leaves the boundary

Run them with:

```bash
cd apps/server/api
TZ=UTC bunx vitest run --config vitest.config.ts src/services/content-gateway/content-gateway.controller.spec.ts
```

```bash
cd apps/server/server
TZ=UTC bunx vitest run src/collections/workflows/system-workflow-runner.service.spec.ts
```
