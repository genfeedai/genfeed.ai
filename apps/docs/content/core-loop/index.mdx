---
description: "Understand Genfeed's trends-to-create-to-publish-to-analytics operating loop and the server modules, product surfaces, and smoke paths behind it."
---
...
flowchart LR
  ...
  H --> I["Stored post analytics\npostAnalytics"]
  I --> J["Analytics aggregation\nPerformanceSummaryService + BusinessAnalyticsService"]
  I --> K["Closed-loop sync\nAnalyticsSyncService -> contentPerformance"]
  J --> L["Dashboard-visible summaries\npost analytics / performance / business views"]
  K --> L
```
...
- **Trend ingestion**: verify `TrendsService` falls through the cached path into the live-fetch path when cache is empty — empty feed when no real data (no synthetic seed)
