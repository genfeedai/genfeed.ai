---
description: 'Run Genfeed Community from a checksummed release bundle with Docker, a pinned application image, PostgreSQL, and one seeded workspace.'
---
...
Run Genfeed Community from the checksummed release bundle with a pinned image,
Postgres, and a single seeded workspace.
...
The bundle pins the exact GHCR image associated with its GitHub release, starts
Postgres, applies Prisma migrations, and seeds one local user, organization, and
brand.
...
Migrations run on startup. Back up first and read the
[upgrade notes](/core/installation#upgrade-notes) for releases that lock tables
while they migrate.
