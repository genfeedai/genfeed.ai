---
description: 'Understand where Genfeed packages, backend services, frontends, and enterprise code live, plus how package publishing ownership is enforced.'
---
...
- **Enterprise packages** live under `ee/packages/*` and use the commercial
  `ee/` license. The repository default outside `ee/` is AGPL-3.0-or-later, but
  package-specific license notices can apply instead.
...
The current `ee/packages/*` tree contains billing and harness packages. Those
packages may consume public packages from `packages/*`, but the Community build
must not import commercial runtime code from `ee/`.
