...
| `BETTER_AUTH_SECRET`      | Auth       | --      | Better Auth signing secret for sessions and JWTs     |
| `BETTER_AUTH_URL`      | Auth       | API URL      | Public origin where the API serves Better Auth routes     |
| `BETTER_AUTH_TRUSTED_ORIGINS`      | Auth       | --      | Comma-separated frontend origins allowed to initiate Better Auth flows     |
| `NEXT_PUBLIC_BETTER_AUTH_ENABLED` | Auth | `true`   | Frontend runtime switch for Better Auth UI and route behavior        |
