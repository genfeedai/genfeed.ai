---
description: "Understand Genfeed's reviewed model registry, OpenRouter text curation, verified Replicate media discovery, and assigned managed models."
---
...
| Agent model filter | `packages/agent/src/hooks/use-agent-registry-models.ts`                      | Filters reviewed registry records for the shared agent model selector                |
...
| Generation tools   | `packages/actions/src/registry/source/index.ts`                                | Exposes stable agent-facing tools such as `generate_image` and `generate_video`      |
...
Agent text-model choices come from reviewed model records. The agent hook limits those records to active text models with agent capabilities, OpenRouter-backed rows, or supported legacy entries before passing them to the shared selector.
...
- Agent tool metadata: `packages/actions/src/registry/source.ts`
- Agent tool registry/UI mapping: `packages/actions/src/registry/tool-registry.ts`
