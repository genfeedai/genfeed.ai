---
description: 'Understand the runtime and data boundaries between self-hosted Genfeed Community and Genfeed Cloud managed accounts, inference, and services.'
---
...
<Callout type="info">
  ...
  handoff. Boundary spec version: v1.3.1.
</Callout>
...
Cloud-only services do not run inside Community unless they are called through
an explicit Cloud API integration. Organization billing code ships in the
Community image but stays inactive at runtime unless `hasOrganizationBilling()`
resolves true.
...
V1 does not provide automatic Core/Cloud record synchronization. Evaluate
export/import support separately for each resource before planning a migration;
do not assume workflows, brands, templates, assets, or configuration have a
complete bidirectional path. Managed execution keeps local records local and
stores enough metadata to identify Cloud-credit-backed runs. Future sync work
must define conflict handling before writing records across both systems.
