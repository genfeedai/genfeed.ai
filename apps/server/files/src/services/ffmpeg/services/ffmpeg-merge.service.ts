...
export class FFmpegMergeService {
  ...
  async mergeNormalizedVideos(inputPaths: string[], outputPath: string, width: number, height: number): Promise<void> {
    if (inputPaths.length < 2 || inputPaths.length > 6 || !Number.isInteger(width) || !Number.isInteger(height) || width < 16 || height < 16 || width > 4096 || height > 4096 || width % 2 || height % 2) throw new Error('Invalid normalized scene composition');
    const paths = await Promise.all(inputPaths.map(async (input) => {
      ...
      if (!probe.streams.some((stream) => stream.codec_type === 'video') || !probe.streams.some((stream) => stream.codec_type === 'audio')) throw new Error('Every scene requires generated video and speech audio');
      return safe;
    }));
    ...
    filters.push(`${paths.map((_, index) => `[v${index}][a${index}]`).join('')}concat=n=${paths.length}:v=1:a=1[v][a]`);
    await this.core.executeFFmpeg(['-y', ...paths.flatMap((input) => ['-i', input]), '-filter_complex', filters.join(';'), '-map', '[v]', '-map', '[a]', '-c:v', 'libx264', '-c:a', 'aac', '-movflags', '+faststart', output]);
  }
