  ...
  describe('listObjects', () => {
    it('should list objects with the given prefix', async () => {
      mockSend.mockResolvedValue({
        Contents: [
          {
            Key: 'trainings/voice-clones/voice-a/model.pth',
            LastModified: new Date('2024-01-01'),
            Size: 1024,
          },
          {
            Key: 'trainings/voice-clones/voice-b/model.pth',
      ...
      const result = await service.listObjects(
        'test-bucket',
        'trainings/voice-clones/',
      );

      expect(result).toHaveLength(2);
      expect(result[0].key).toBe('trainings/voice-clones/voice-a/model.pth');
      expect(result[0].size).toBe(1024);
      expect(result[1].key).toBe('trainings/voice-clones/voice-b/model.pth');
    });

    it('should handle pagination', async () => {
      mockSend
        .mockResolvedValueOnce({
          Contents: [{ Key: 'file1.wav', LastModified: new Date(), Size: 100 }],
          NextContinuationToken: 'token-123',
        })
        .mockResolvedValueOnce({
          Contents: [{ Key: 'file2.wav', LastModified: new Date(), Size: 200 }],
          NextContinuationToken: undefined,
        });
