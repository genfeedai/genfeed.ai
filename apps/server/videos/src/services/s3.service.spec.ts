...
vi.mock('node:fs/promises', () => ({
  ...
  readFile: vi.fn().mockResolvedValue(Buffer.from('test-video-data')),
  ...
  describe('downloadFile', () => {
    it('should download a file from S3 to local path', async () => {
      ...
      await service.downloadFile(
        'test-bucket',
        'videos/generated/abc/output.mp4',
        '/tmp/output.mp4',
      );
  ...
  describe('uploadFile', () => {
    it('should upload a file to S3', async () => {
      ...
      await service.uploadFile(
        'test-bucket',
        'videos/generated/job123/video.mp4',
        '/tmp/video.mp4',
      );
  ...
  describe('listObjects', () => {
    it('should list objects with the given prefix', async () => {
      mockSend.mockResolvedValue({
        Contents: [
          {
            Key: 'videos/generated/job1/output.mp4',
            LastModified: new Date('2024-01-01'),
            Size: 1048576,
          },
          {
            Key: 'videos/generated/job2/output.mp4',
      ...
      expect(result[0].key).toBe('videos/generated/job1/output.mp4');
      expect(result[0].size).toBe(1048576);
      expect(result[1].key).toBe('videos/generated/job2/output.mp4');
    });
    ...
    it('should handle pagination', async () => {
      mockSend
        .mockResolvedValueOnce({
          Contents: [
            { Key: 'video1.mp4', LastModified: new Date(), Size: 100000 },
          ],
          NextContinuationToken: 'token-123',
        })
        .mockResolvedValueOnce({
          Contents: [
            { Key: 'video2.mp4', LastModified: new Date(), Size: 200000 },
          ],
          NextContinuationToken: undefined,
        });

      const result = await service.listObjects('test-bucket', 'videos/');
