...
function setup() {
        ...
        async ({ where }) =>
          [...executions.values()].find(
            (row) =>
              ...
              (row.result as { metadata: Row }).metadata.threadId ===
                where.result.equals,
          ) ?? null,
      ),
  ...
  let threadCount = 0;
  const threads = {
    create: vi.fn(async () => ({ id: `thread-${++threadCount}` })),
  };
...
describe('proactive dispatch failure accounting across real services', () => {
    ...
    expect(fixture.prisma.workflowExecution.findFirst).toHaveBeenLastCalledWith(
      {
        where: {
          organizationId: 'org',
          isDeleted: false,
          result: { path: ['metadata', 'threadId'], equals: 'thread-3' },
        },
        select: { id: true },
      },
    );
  ...
  it('counts thread creation failure once without inventing a terminal run', async () => {
    const fixture = setup();
    fixture.threads.create.mockRejectedValueOnce(
    ...
    expect(fixture.prisma.workflowExecution.findFirst).not.toHaveBeenCalled();
  });
