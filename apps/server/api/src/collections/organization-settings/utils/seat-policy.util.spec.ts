...
describe('seat-policy.util', () => {
  describe('isUnlimitedSeatTier', () => {
    it('is true for Cloud Teams (scale) and Enterprise', () => {
    ...
    it('is false for free/creator/pro/byok tiers', () => {
      expect(isUnlimitedSeatTier(SubscriptionTier.FREE)).toBe(false);
      expect(isUnlimitedSeatTier(SubscriptionTier.CREATOR)).toBe(false);
      expect(isUnlimitedSeatTier(SubscriptionTier.PRO)).toBe(false);
  ...
  describe('resolveEffectiveSeatsLimit', () => {
    it('returns unlimited (null) for Cloud Teams regardless of stored limit', () => {
    ...
    it('honors the stored fair-use limit for finite tiers', () => {
      expect(resolveEffectiveSeatsLimit(SubscriptionTier.FREE, 3)).toBe(3);
      expect(resolveEffectiveSeatsLimit(SubscriptionTier.CREATOR, 5)).toBe(5);
      expect(resolveEffectiveSeatsLimit(SubscriptionTier.PRO, 10)).toBe(10);
