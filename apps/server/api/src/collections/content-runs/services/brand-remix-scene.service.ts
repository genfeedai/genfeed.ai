...
export class BrandRemixSceneService {
  constructor(private readonly store: BrandRemixSceneStoreService, private readonly quotes: BrandRemixSceneQuoteService, private readonly source: BrandRemixSceneSourceService, private readonly workflow: BrandRemixSceneWorkflowService, private readonly credits: CreditsUtilsService) {}
  private editable(config: BrandRemixRunConfig) {
    ...
    if (isSceneOperationActive(config.scenePipeline)) throw new ConflictException('Cancel or finish the active scene operation first.');
  }
  async attachSource(organizationId: string, runId: string, user: AuthenticatedUser, body: unknown): Promise<BrandRemixRunView> {
    const input = attachBrandRemixAnalysisSourceSchema.parse(body);
    const { config, brandId } = await this.store.read(organizationId, runId, input.expectedRevision);
    ...
    if (asset && config.analysisSource?.assetId === asset.sourceAssetId && config.analysisSource.assetUpdatedAt === asset.assetUpdatedAt) return this.store.view(organizationId, runId);
    const replacedAssetIds = [...(pipeline?.replacedAssetIds ?? []), ...Object.values(pipeline?.scenes ?? {}).flatMap((scene) => [scene.image.assetId, scene.video.assetId]), pipeline?.assembly?.assetId, pipeline?.assembly?.mergedAssetId].filter((id): id is string => Boolean(id));
    await this.store.save(organizationId, runId, config, { ...config, revision: config.revision + 1, generationQuote: undefined, phase: 'prefilled', execution: undefined, analysisSource: asset ? { assetId: asset.sourceAssetId, assetUpdatedAt: asset.assetUpdatedAt, selectedByUserId: user.userId, selectedAt: new Date().toISOString(), selection: 'brand_library', purpose: 'analysis_only' } : undefined, concept: config.concept ? { ...config.concept, storyboard: config.concept.storyboard.map(({ sourceObservation: _observation, ...scene }) => scene) } : undefined, scenePipeline: { ...initialScenePipeline(), cancellationGeneration: (pipeline?.cancellationGeneration ?? 0) + 1, receipts: pipeline?.receipts ?? [], replacedAssetIds: [...new Set(replacedAssetIds)] } });
    return this.store.view(organizationId, runId);
  }
  async quote(organizationId: string, runId: string, _user: AuthenticatedUser, body: unknown): Promise<BrandRemixRunView> {
    const input = quoteBrandRemixScenesSchema.parse(body);
    const { config, brandId } = await this.store.read(organizationId, runId, input.expectedRevision);
    this.editable(config);
    const next = { ...config, ...(config.concept ? { concept: stableSceneConcept(config.concept) } : {}), scenePipeline: config.scenePipeline ?? initialScenePipeline() };
    const quote = await this.quotes.build(organizationId, brandId, next, input);
    await this.store.save(organizationId, runId, config, { ...next, scenePipeline: { ...next.scenePipeline, quote, state: 'quoted' } });
    return this.store.view(organizationId, runId);
  }
  async execute(organizationId: string, runId: string, user: AuthenticatedUser, _request: RequestWithContext, body: unknown): Promise<BrandRemixRunView> {
    const input = executeBrandRemixScenesSchema.parse(body);
    const { config, brandId } = await this.store.read(organizationId, runId, input.expectedRevision);
    const pipeline = config.scenePipeline;
    if (pipeline?.operation?.quoteId === input.quoteId) return this.store.view(organizationId, runId);
    this.editable(config);
    ...
    if (quote.operation === 'analysis') {
      ...
      next.analysis = { sourceAssetId: source.sourceAssetId, durationSeconds: source.durationSeconds, sizeBytes: source.sizeBytes, model: LLM_DEFAULTS.background, transcript: pipeline.analysis?.transcript, srt: pipeline.analysis?.srt, transcription: pipeline.analysis?.transcript ? { ...pipeline.analysis.transcription, state: 'ready' } : { attempt: quote.items.find((item) => item.stage === 'transcription')?.attempt ?? 1, state: 'pending' }, rewrite: { attempt: quote.items.find((item) => item.stage === 'analysis')?.attempt ?? 1, state: 'pending' }, keyframes: [], vendorCostKnown: false };
    } else {
      next.state = 'generating';
      if (next.assembly?.assetId) next.replacedAssetIds.push(next.assembly.assetId);
  ...
  async cancel(organizationId: string, runId: string, body: unknown): Promise<BrandRemixRunView> {
  ...
  async resume(organizationId: string, runId: string, _user: AuthenticatedUser, body: unknown): Promise<BrandRemixRunView> {
    const input = controlBrandRemixScenesSchema.parse(body);
    const { config } = await this.store.read(organizationId, runId, input.expectedRevision);
    const pipeline = config.scenePipeline;
