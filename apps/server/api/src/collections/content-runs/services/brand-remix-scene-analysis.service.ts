  ...
  async step(
    ...
    try {
      ...
      if (stageName === 'transcription') {
        ...
        if (!saved?.analysis)
          throw new ConflictException('Analysis disappeared.');
      ...
      const parts: OpenRouterMessageContentPart[] = [
        {
          ...
          text: JSON.stringify({
            ...
            authorizedReferenceRoles: config.draft.references.map(
              ({ role, description }) => ({ role, description }),
            ),
            objective: config.draft.intent,
        ...
        await this.store.save(organizationId, runId, current.config, {
          ...
          scenePipeline: {
            ...
            analysis: {
              ...saved.analysis,
              [stageName]: {
                ...saved.analysis[stageName],
                state: returned || !dispatched ? 'failed' : 'uncertain',
                error:
