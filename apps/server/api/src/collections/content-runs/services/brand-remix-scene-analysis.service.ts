  ...
  async step(organizationId: string, runId: string, operationId: string): Promise<boolean> {
    ...
    return true;
      ...
      if (saved?.operation?.id === operationId && saved.analysis) {
        await this.store.save(organizationId, runId, current.config, { ...current.config, scenePipeline: { ...saved, analysis: { ...saved.analysis, [stageName]: { ...saved.analysis[stageName], state: returned || !dispatched ? 'failed' : 'uncertain', error: error instanceof Error ? error.message : 'Analysis failed' } } } });
      }
