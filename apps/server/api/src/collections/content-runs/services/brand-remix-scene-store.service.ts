...
export class BrandRemixSceneStoreService {
  constructor(private readonly persistence: BrandRemixRunPersistenceService, private readonly planning: BrandRemixRunPlanningService) {}
  async read(organizationId: string, runId: string, expectedRevision?: number) {
    ...
    if (expectedRevision !== undefined && expectedRevision !== config.revision) throw new ConflictException('The remix changed. Reload and retry.');
    return { run, config, brandId: this.persistence.requireBrandId(run) };
  }
  async save(organizationId: string, runId: string, previous: BrandRemixRunConfig, next: BrandRemixRunConfig) {
    const nextConfig = brandRemixRunConfigSchema.parse(next);
    const run = await this.persistence.compareAndSwapExactConfig({ expectedConfig: previous, nextConfig, organizationId, runId, status: next.phase === 'ready_for_review' ? ContentRunStatus.COMPLETED : next.scenePipeline && ['analysing', 'generating', 'assembling'].includes(next.scenePipeline.state) ? ContentRunStatus.RUNNING : ContentRunStatus.PENDING });
    if (!run) throw new ConflictException('The remix changed during this operation. Reload and retry.');
    return nextConfig;
  }
  async view(organizationId: string, runId: string) {
    const { run, config, brandId } = await this.read(organizationId, runId);
    return projectBrandRemixRun(run, await this.planning.resolveBrandContext(organizationId, brandId), config);
  }
  async fence(organizationId: string, runId: string, operationId: string) {
    ...
    if (!pipeline?.operation || pipeline.operation.id !== operationId || pipeline.state === 'cancelled' || pipeline.operation.revision !== saved.config.revision || pipeline.operation.cancellationGeneration !== pipeline.cancellationGeneration) throw new ConflictException('Scene operation was cancelled or superseded.');
    return saved;
  }
}
