...
describe('Subscription billing HTTP composition', () => {
  ...
  let changeSubscriptionPlan: MockFn;
      ...
      providers: [
        {
          ...
          useValue: { changeSubscriptionPlan, previewSubscriptionChange },
        },
  ...
  describe('POST /subscriptions/current/preview', () => {
  ...
  describe('PATCH /subscriptions/current', () => {
    ...
    it('renders a billing divergence as a reported 500 that never advertises a retry', async () => {
      changeSubscriptionPlan.mockRejectedValue(
        new SubscriptionChangeException(
          SubscriptionChangeFailureCode.PLAN_CHANGE_NOT_RECORDED,
          new Error('connection terminated provider-secret-token'),
        ),
      );
    ...
    it('reports a durable change whose credit reset failed as a success that says so', async () => {
      changeSubscriptionPlan.mockResolvedValue({
        creditsOutcome: SubscriptionPlanChangeCreditsOutcome.FAILED,
        stripeSubscription: { id: 'sub_stripe_1' },
        subscription: { id: 'sub_row_1' },
      });
      ...
      expect(response.body.message).toContain('could not be reset');
      expect(response.body.data.creditsOutcome).toBe('failed');
    });
  });
});
