...
describe('XAdWatchedAdvertisersService persistence boundary', () => {
  let delegate: MockDelegate;
  let service: XAdWatchedAdvertisersService;
  ...
  beforeEach(() => {
    ...
    service = new XAdWatchedAdvertisersService(
      {
  ...
  describe('create', () => {
    it('writes only defined scalar fields when the handle is not already watched', async () => {
      ...
      expect(delegate.create).toHaveBeenCalledWith({
        data: { advertiserHandle: 'nike', organizationId: 'org-1' },
      });
  ...
  describe('findByHandle', () => {
    it('scopes to active rows by default', async () => {
      await service.findByHandle('org-1', 'nike');
    ...
    it('includes soft-deleted rows when asked, for the revive-on-recreate lookup', async () => {
      await service.findByHandle('org-1', 'nike', null, {
        includeDeleted: true,
      });
    ...
    it('includes the brand in the natural-key lookup', async () => {
      await service.findByHandle('org-1', 'nike', 'brand-1');
    ...
    it('records only a safe error code and does not advance successful snapshot fields', async () => {
      ...
      expect(delegate.updateMany).toHaveBeenCalledWith({
        data: {
          freshnessState: 'unavailable',
          lastAttemptedAt: expect.any(Date),
          lastIngestionErrorCode: 'x_ads_repository_contract_fixtures_missing',
