/**
 * Compiles a canonical image generation brief into a FLUX 2 Dev dispatch
 * payload. The `input_images` field is optional and accepts up to 4
 * references — every included reference maps onto it as an array; anything
 * beyond the profile's max is recorded as an omitted signal. FLUX 2 Dev has
...
import {
  ...
  flux2DevDispatchSchema,
} from '@genfeedai/contracts/api-types/contracts/generation-brief-compiler.contract';
import { FLUX_2_DEV_CAPABILITY_PROFILE } from '@genfeedai/contracts/api-types/contracts/generation-capability-profile.contract';

const FLUX_2_DEV_MODEL_LABEL = 'FLUX 2 Dev';

export interface CompileFlux2DevGenerationBriefInput {
  brief: ImageGenerationBrief;
  modelKey: string;
  seed?: number;
}

export interface Flux2DevCompileResult {
  brief: ImageGenerationBrief;
  dispatch: Flux2DevDispatch;
  evidence: GenerationBriefCompileEvidence;
}

export function compileFlux2DevGenerationBrief(
  input: CompileFlux2DevGenerationBriefInput,
): Flux2DevCompileResult {
  const { brief, modelKey, seed } = input;
  const profile = FLUX_2_DEV_CAPABILITY_PROFILE;
  if (modelKey !== profile.modelKey) {
    throw new GenerationBriefCompileError(
      `FLUX 2 Dev compiler received an unregistered model key: ${modelKey}.`,
      'invalid_brief',
    );
  }
  ...
  if (brief.mediaKind !== 'image') {
    throw new GenerationBriefCompileError(
      'FLUX 2 Dev compiler only supports image briefs.',
      'invalid_brief',
    );
  }
  ...
  const dispatch = flux2DevDispatchSchema.parse({
