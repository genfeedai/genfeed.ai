...
export class EmailPerformanceService {
  ...
  async deliverClaimed(deliveryId: string): Promise<void> {
    ...
    try {
      ...
      const billingAccess =
        !membership &&
        (row.topic === 'billing.credits' || row.topic === 'billing.receipt')
      ...
      await this.prisma.$transaction(async (tx) => {
        ...
        await tx.emailMessage.updateMany({
          ...
          data: { acceptedAt: now, providerMessageId },
        });
        if (legacyId)
          await tx.lifecycleEmailDelivery.updateMany({
            where: {
        ...
        await tx.notificationDelivery.updateMany({
          ...
          data: {
            ...
            nextAttemptAt: new Date(
              Date.now() + Math.min(3_600_000, 30_000 * 2 ** row.attemptCount),
            ),
          },
        });
        if (terminal && legacyId)
          await tx.lifecycleEmailDelivery.updateMany({
            where: {
              id: legacyId,
              userId: row.userId,
