...
export class AgentOrchestratorUiActionMutationService {
  ...
  async execute(
    ...
    if (
      !scope ||
      proposal.data.scopeVersion !== scope.contextVersion ||
      (proposal.data.brandId ?? null) !== (scope.brandId ?? null)
    ) {
    ...
    const actions = proposal.actions.map((candidate) => {
      const card = record(candidate);
      return card.id === sourceActionId
        ? {
            ...card,
            ctas: [],
    ...
    const patched = await this.messages.patchAll(
      ...
      {
        metadata: { ...proposal.metadata, uiActions: actions },
      },
    );
    if (patched.modifiedCount !== 1 && proposal.data.status !== status)
      throw new ConflictException(
  ...
  private async loadProposal(
    ...
    for (let page = 1; page <= 50; page++) {
      ...
      for (const message of messages) {
        ...
        return { actions, data, messageId: String(message.id), metadata };
      }
