/**
 * @fileoverview Tests for LinkedInPublisherService
...
describe('LinkedInPublisherService', () => {
  ...
  describe('publish', () => {
    ...
    describe('image posts', () => {
      it('should publish a single image successfully', async () => {
        ...
        const mockActivityId = 'urn:li:activity:1234567890';

        linkedInService.uploadImage.mockResolvedValue({
          id: mockActivityId,
        });
      ...
      it('should handle HTML in description by converting to plain text', async () => {
        ...
        expect(result.success).toBe(true);
        expect(linkedInService.uploadImage).toHaveBeenCalled();
      });
    });
    ...
    describe('video posts', () => {
      it('should publish a video successfully', async () => {
        ...
        const mockActivityId = 'urn:li:activity:video123';

        linkedInService.uploadVideo.mockResolvedValue({
          id: mockActivityId,
        });
        ...
        expect(result.success).toBe(true);
        expect(result.externalId).toBe(mockActivityId);
        expect(linkedInService.uploadVideo).toHaveBeenCalledWith(
          mockOrganizationId.toString(),
          mockBrandId.toString(),
    ...
    describe('error handling', () => {
      ...
      it('should throw error when LinkedIn API fails', async () => {
        const context = createPublishContext(mockImagePost);
        const error = new Error('LinkedIn API error');

        linkedInService.uploadImage.mockRejectedValue(error);
  ...
  describe('buildPostUrl', () => {
    it('should build correct LinkedIn URL using activity URN', () => {
      const externalId = 'urn:li:activity:1234567890';
      ...
      expect(result).toBe(`https://www.linkedin.com/feed/update/${externalId}`);
    });

    it('should handle any external ID format', () => {
      const externalId = 'custom-id-format';
  ...
  describe('publishThreadChildren', () => {
    const mockParentExternalId = 'urn:li:activity:parent123';
