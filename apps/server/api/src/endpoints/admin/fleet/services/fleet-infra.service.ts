  ...
  getEC2Status(): Promise<AdminFleetEC2InstanceStatus[]> {
    ...
    return this.runAws(
      ...
      async () => {
        ...
        const instances: AdminFleetEC2InstanceStatus[] = [];
  ...
  invalidateCloudFront(
    ...
    if (!distributionId) {
      throw new BadRequestException(
        'CloudFront distribution ID is not configured for fleet invalidation',
      );
    }
    this.loggerService.log(caller, {
      distributionId,
    ...
    return this.runAws(
      ...
      async () => {
        const command = new CreateInvalidationCommand({
          ...
          InvalidationBatch: {
            CallerReference: `fleet-${Date.now()}`,
            Paths: {
