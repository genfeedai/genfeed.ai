...
/**
 * Owns fleet pipeline aggregation (campaigns + pipeline stats). Reads
 * aggregates through the owning collection services rather than touching their
 * Prisma client directly.
 */
@Injectable()
export class AdminFleetStatsService {
  ...
  async getPipelineStats(organizationId: string): Promise<{
    ...
    for (const item of assetStatusGroups) {
      byStatus[AdminFleetValueReader.readString(item.status) ?? 'unknown'] =
        item.count;
    }
    ...
    for (const item of assetReviewStatusGroups) {
      byReviewStatus[
        AdminFleetValueReader.readString(item.reviewStatus) ?? 'unknown'
      ] = item.count;
    }

    const byStage: Record<string, number> = {};
    for (const item of trainingStageGroups) {
      byStage[AdminFleetValueReader.readString(item.stage) ?? 'unknown'] =
        item.count;
    }
