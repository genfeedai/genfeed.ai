...
/**
 * Auth CLI controller.
 * Provides a browser-authenticated endpoint that exchanges a Better Auth session
 * for a long-lived API key the CLI can store locally.
 *
 * Flow: CLI opens localhost callback server -> browser redirects here with
 * Better Auth session -> endpoint returns plain API key -> CLI stores it.
 */
@ApiTags('Auth')
@Controller('auth')
@ApiBearerAuth()
export class AuthCliController {
  ...
  @ApiOperation({
    summary: 'Exchange Better Auth session for a CLI API key',
  })
  @ApiResponse({
