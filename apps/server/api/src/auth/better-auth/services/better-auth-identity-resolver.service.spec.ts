...
describe('BetterAuthIdentityResolverService', () => {
  ...
  let membersService: {
    findActiveForIdentityBootstrap: ReturnType<typeof vi.fn>;
  };
  let identityCache: {
  ...
  beforeEach(() => {
    ...
    membersService = {
      findActiveForIdentityBootstrap: vi.fn().mockResolvedValue([]),
    };
    identityCache = {
  ...
  it('does not treat organization admins as platform superadmins', async () => {
    ...
    membersService.findActiveForIdentityBootstrap.mockResolvedValue([
      {
  ...
  it('returns the cached identity without touching the database on a cache hit', async () => {
    ...
    expect(
      membersService.findActiveForIdentityBootstrap,
