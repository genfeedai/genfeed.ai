...
} from '@server/collections/agent-memories/services/agent-memories.service';
...
import { buildPageContextPrompt } from '@server/services/agent-orchestrator/utils/agent-page-context.util';
import {
  applyAgentReplyStyle,
  buildAgentSystemPrompt,
} from '@server/services/agent-orchestrator/utils/agent-system-prompt.util';
import {
  fenceUntrustedContent,
  sanitizeAgentUntrustedInput,
  UNTRUSTED_USER_DATA_FRAMING,
} from '@server/services/agent-orchestrator/utils/agent-untrusted-content.util';
...
function buildGenerationModePrompt(
  ...
  if (mode === 'image') {
    return 'The operator explicitly selected Image mode for this turn (generationType=image). Generate the image directly in this turn using the available visual-generation tool. Do not ask for another confirmation and do not choose video.';
  }
  if (mode === 'video') {
    return 'The operator explicitly selected Video mode for this turn (generationType=video). Generate the video directly in this turn using the available visual-generation tool. Do not ask for another confirmation and do not choose image.';
  }
  return '';
}
