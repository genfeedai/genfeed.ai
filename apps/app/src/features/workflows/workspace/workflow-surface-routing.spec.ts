...
describe('workflow surface routing', () => {
  it('resolves focused graph editors under automate/workflows', () => {
    expect(
      resolveWorkflowSurfaceRoute(
      ...
      executionId: 'run-1',
      isGraphCanvas: true,
      workflowBaseHref: '/acme/moonrise/automate/workflows',
  ...
  it('preserves opaque queries while restoring the connected thread', () => {
    expect(
      appendWorkflowThread(
        '/acme/moonrise/automate/workflows/workflow-1?execution=run-1',
        'thread-1',
      ),
    ).toBe(
      '/acme/moonrise/automate/workflows/workflow-1?execution=run-1&thread=thread-1',
    );
  });
});
