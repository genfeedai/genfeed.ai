...
export default function CharacterDetail({ slug }: CharacterDetailProps) {
  ...
  } = useQuery<FleetCharacterRecord>({
    queryKey: ['fleet-character', slug],
    queryFn: async () => {
      try {
        const service = await getFleetService();
        return service.getCharacter(slug) as Promise<FleetCharacterRecord>;
      } catch (error) {
        logger.error(`GET /admin/fleet/characters/${slug} failed`, error);
        throw error;
      }
    },
  });
  ...
  } = useQuery<IFleetAsset[]>({
    queryKey: ['fleet-assets', slug],
    queryFn: async () => {
      try {
        const service = await getFleetService();
        ...
        logger.error(
          `GET /admin/fleet/assets?personaSlug=${slug} failed`,
          error,
        );
        throw error;
      }
  ...
  const handleDelete = useCallback(
    ...
    [getFleetService, notificationsService, refreshAssets],
  );
