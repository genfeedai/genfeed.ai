 ...
 * `type` field on a unified content row. The Publish desk hosts all three under
 * `/publish/posts/:id`, so the editor is chosen by **which entity the id
 ...
 */
export async function resolvePublishContentKindFromId(
  contentId: string,
  services: {
  ...
  signal?: AbortSignal,
): Promise<PublishContentKind | null> {
  const probes: Array<{
...
/** @deprecated Prefer resolvePublishContentKindFromId — query params are not source of truth. */
export function resolvePublishContentKindFromQuery(
  value: string | null,
