/**
 * The Genfeed mark, inlined for satori.
 *
 * A mirror of the canonical CDN object (`/assets/branding/logo.svg`), the same
 * arrangement `apps/app/public/logo.svg` already uses — refresh it from the CDN
 * rather than editing the path by hand. It is inlined instead of fetched
 * because a social card that loses a network round trip should still carry the
 * brand: a failed fetch here would drop the mark with no other symptom.
 *
 * The source file is filled black for light chrome; the card is dark, so the
 * fill is a prop rather than baked in.
 *
 * `aria-hidden` is the honest label: satori rasterises this to PNG, so there
 * is no accessibility tree to reach and the card's alt text is the page's
 * `og:image:alt`, not anything in here.
 */
export function BrandMark({ fill, size }: { fill: string; size: number }) {
  return (
    <svg
      aria-hidden="true"
      fill={fill}
