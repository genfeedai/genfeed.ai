...
describe('DesktopCloudService', () => {
  ...
  it('routes non-hook desktop content through generic Genfeed generation', async () => {
    globalThis.fetch = (async (
      input: RequestInfo | URL,
      init?: RequestInit,
    ) => {
      expect(String(input)).toBe(
        'https://api.genfeed.ai/v1/content-intelligence/generate',
      );
      expect(init?.method).toBe('POST');
      expect(init?.headers).toMatchObject({
        Authorization: 'Bearer gf_desktop_key',
      ...
      expect(init?.body).toBe(
        JSON.stringify({
          additionalContext: [
            'Desktop content type: article',
            'Publish intent: review',
            'Source trend: Desktop launch trend',
          ],
          platform: 'linkedin',
          templateCategory: 'article',
          topic: 'Desktop launch trend',
          variationsCount: 1,
        }),
      );
      ...
      return new Response(
        JSON.stringify({
          data: [
            {
              attributes: {
                content: 'A full article generated by Genfeed.',
              },
              id: 'generated-0',
            },
          ],
    ...
    const result = await service.generateContent({
      ...
      prompt: 'Launch Genfeed Desktop',
      publishIntent: 'review',
      sourceTrendTopic: 'Desktop launch trend',
      type: 'article',
    });
