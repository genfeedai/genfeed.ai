...
Currently configured project: `genfeed-ai` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/genfeed-ai/SETUP.md` to fill in
   `data/genfeed-ai/INFO.md`.
...
`--project-id genfeed-ai` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/genfeed-ai/`.
