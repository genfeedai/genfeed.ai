 ...
 *
 * Prerequisites:
 * - LEGACY_MONGODB_URI in cloud/apps/server/api/.env.{local|production}
 *
 * Usage:
...
async function main() {
  const sourceDbName = getSourceDbName(LEGACY_MONGODB_URI);
  ...
  // Connect to source
  const sourceClient = new MongoClient(LEGACY_MONGODB_URI);
