...
import {
  createAwsGateway,
  createTasksStoppedWaiterConfig,
  type EcsGateway,
  EXIT,
  isTransientError,
...
interface FakeGateway extends EcsGateway {
  ...
  lastRunTaskParams: RunTaskParams | null;
}
...
function makeGateway(config: FakeGatewayConfig = {}): FakeGateway {
  ...
  return {
    calls,
    lastRunTaskParams: null,
    async runTask(params) {
      calls.runTask += 1;
      this.lastRunTaskParams = params;
...
describe('runTaskAndWait', () => {
  ...
  test('forwards a container command override to the ECS gateway', async () => {
    const gw = makeGateway();
    const command = [
      'node',
    ...
    const result = await runTaskAndWait(
      ...
      baseOptions({
        command,
        containerName: 'articles-seed',
        phase: 'articles-seed-live',
      }),
    );

    expect(result.code).toBe(EXIT.SUCCESS);
    expect(gw.lastRunTaskParams).toMatchObject({
      command,
      containerName: 'articles-seed',
    });
  });
...
describe('parseCliArgs', () => {
  ...
  test('parses a paired container command override', () => {
    const opts = parseCliArgs([
      ...fullArgs,
      '--container-name',
      'articles-seed',
      '--command-arg=node',
      '--command-arg=apps/server/dist/apps/api-articles-seed/main',
      '--command-arg=--live',
    ]);

    expect(opts.containerName).toBe('articles-seed');
  ...
  test('rejects an incomplete container command override', () => {
    expect(() =>
      parseCliArgs([...fullArgs, '--container-name', 'articles-seed']),
    ).toThrow(UsageError);
    expect(() => parseCliArgs([...fullArgs, '--command-arg=node'])).toThrow(
      UsageError,
    );
  });
...
describe('createAwsGateway.runTask', () => {
  test('maps a container command override into the ECS request', async () => {
    let input: unknown;
    const client = {
    ...
    await createAwsGateway(client).runTask({
      ...
      securityGroups: ['sg-1'],
      subnets: ['subnet-1'],
      taskDefinition: 'genfeed-production-articles-seed:1',
    });
...
describe('createTasksStoppedWaiterConfig', () => {
  test('keeps the ECS waiter on a fixed cadence for the full timeout budget', () => {
    const client = {} as ECSClient;

    expect(createTasksStoppedWaiterConfig(client, 600)).toEqual({
      client,
      maxDelay: 6,
      maxWaitTime: 600,
      minDelay: 6,
