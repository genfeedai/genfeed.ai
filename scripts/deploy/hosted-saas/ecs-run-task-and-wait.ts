 ...
 * to live inline in the private hosted-SaaS deploy workflow (migrate, workflow
 ...
 * SDK's native `waitUntilTasksStopped` waiter (configurable max-wait with a
 * fixed 6s poll cadence) plus a single, unit-tested classification of the
 * terminal state into a documented exit-code enumeration.
 *
 ...
 *   bun scripts/deploy/hosted-saas/ecs-run-task-and-wait.ts \
...
const TASKS_STOPPED_POLL_SECONDS = 6;
...
/**
 * Keep task polling on the ECS waiter's documented six-second cadence.
 *
 * The generated waiter otherwise backs off exponentially to a 600-second
 * maxDelay. Smithy's waiter exits early when the next backoff would cross the
 * deadline, so a nominal 600-second wait can time out after only a handful of
 * polls. A fixed cadence makes maxWaitTime the actual timeout contract.
 */
export function createTasksStoppedWaiterConfig(
  client: ECSClient,
  maxWaitTime: number,
) {
  return {
    client,
    maxDelay: TASKS_STOPPED_POLL_SECONDS,
    maxWaitTime,
    minDelay: TASKS_STOPPED_POLL_SECONDS,
  };
}
...
export async function runTaskAndWait(
  ...
  // 2. Wait for STOPPED via the AWS-native waiter on a fixed 6s cadence.
    ...
    async runTask(params) {
      const res = await client.send(
        new RunTaskCommand({
          ...
          overrides:
            params.containerName && params.command
              ? {
                  containerOverrides: [
                    {
    ...
    async waitUntilStopped({ cluster, taskArn, maxWaitTimeSeconds }) {
      try {
        await waitUntilTasksStopped(
          createTasksStoppedWaiterConfig(client, maxWaitTimeSeconds),
...
export function parseCliArgs(argv: string[]): RunTaskAndWaitOptions {
  ...
  const containerName = values['container-name'];
  const command = values['command-arg'];
  ...
  if (Boolean(containerName) !== Boolean(command?.length)) {
    throw new UsageError(
      '--container-name and at least one --command-arg must be provided together',
    );
  }
